...
| **B5 / B7** | Delete unused deps; justify keepers in one line |
| **B6** | One error path; user-safe copy + recovery; log detail server-side |
| **S2 / S3** | Fix README/setup/docs so a stranger boots; don’t paper over with “works on my machine” |
| **S4** | Fix root findings; re-run full scoreboard |
| **S5** | Stabilize flakes at root; test critical paths; don’t delete/skip to green |
| **S6** | Change product **or** change the claim |
| **S7** | Record what users should know |
| **S8** | Restore real backup in a clean room once |
